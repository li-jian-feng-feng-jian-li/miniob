...
//try to implement drop_table function @author ljf
RC Db::drop_table(const char *table_name){
  RC rc = RC::SUCCESS;
  Table *table_to_drop = find_table(table_name);
  if(table_to_drop == nullptr) return RC::SCHEMA_TABLE_NOT_EXIST;
  std::string table_file_path = table_meta_file(path_.c_str(),table_name);
  
  //add drop function in table.h,table.cpp,with return value RC 
  rc = table_to_drop->drop(path_.c_str());
  if(rc != RC::SUCCESS){
