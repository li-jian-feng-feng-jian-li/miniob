...
// update
RC MvccTrx::update_record(Table *table, Record &old_record, std::vector<Value> value,std::vector<const char *> field_name)
{
  Field begin_field;
  Field end_field;
  trx_fields(table, begin_field, end_field);
  ...
  RC rc = log_manager_->append_log(CLogType::UPDATE, trx_id_, table->table_id(), old_record.rid(), 0, 0, nullptr);
  ASSERT(rc == RC::SUCCESS,
      "failed to append update record log. trx id=%d, table id=%d, rid=%s, old_record len=%d,rc=%s",
      trx_id_,
      table->table_id(),
      old_record.rid().to_string().c_str(),
      old_record.len(),
