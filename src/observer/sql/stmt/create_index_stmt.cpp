...
RC CreateIndexStmt::create(Db *db, const CreateIndexSqlNode &create_index, Stmt *&stmt)
{
  ...
  for (int i = 0; i < create_index.attribute_name.size(); i++) {
    if (is_blank(table_name) || is_blank(create_index.index_name.c_str()) ||
        is_blank(create_index.attribute_name[i].c_str())) {
      LOG_WARN("invalid argument. db=%p, table_name=%p, index name=%s, attribute name=%s",
         db, table_name, create_index.index_name.c_str(), create_index.attribute_name[i].c_str());
  ...
  std::vector<const FieldMeta *> *vec = new std::vector<const FieldMeta *>;
  ...
  stmt = new CreateIndexStmt(table, vec, create_index.index_name, create_index.is_unique);
  return RC::SUCCESS;
}
